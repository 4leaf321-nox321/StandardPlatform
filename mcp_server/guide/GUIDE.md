...
코어 온톨로지(`pipeline/core/core-ontology.json`)는 **모든 그룹이 함께 쓴다.** 복사본끼리 잇고
합치는 자리가 여기다.

| 타입 | 무엇 | 식별자 |
| --- | --- | --- |
| `project` 과제 | 기간과 목표가 있는 일 한 건(과제 · 프로젝트 · 의뢰) | 과제번호 |
...
`participates` 참여 · `produces` 산출 문서 · `evidence` **근거 문서**(어느 타입에서나) · `uses`
사용 · `involves_org` 관계 기관 · `affiliated` 소속 · `about` 기술 분야.

- **새 것이 코어의 무엇과 같은지 먼저 본다.** 「해석 과제」 는 새 타입이 아니라 과제 + 기술 분야
  (구조 해석)다. 「해석 보고서」 는 문서 + 문서 종류(보고서).
- 그룹 전용 타입 slug 는 **`<그룹코드>_`** 로 시작한다(`cae_load_case`). 코어의 slug · 속성 키 ·
  고를 값의 뜻은 바꾸지 않는다.
- 코어 타입에 그룹이 속성을 **더하는** 것은 된다. 그 속성 키도 `<그룹코드>_` 로 시작한다.
...
| 다른 타입의 것 **하나**를 가리키고, 그 연결에 설명이 필요 없다 | **참조 칸**(`object_ref`) — 소프트웨어의 개발사 |
| 연결 자체에 뜻 · 근거가 있다, 여럿과 잇는다, 양쪽에서 찾아본다 | **관계** — 참여 · 사용 · 근거 문서 |
...
**같은 사실을 두 곳에 두지 않는다.** 참조 칸과 관계 중 하나만 — 둘 다 두면 어긋나고, 조건
