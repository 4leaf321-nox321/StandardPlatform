...
describe('선택할 값 다루기', () => {
  ...
  it('이름 변경 — 몇 개가 함께 바뀌는지 본 뒤에 바꾼다', async () => {
    ontologyApi.renameOption
      .mockResolvedValueOnce({
        applied: false,
        from_value: '스틸',
        to_value: '강',
        objects_with_value: 7,
        errors: [],
      })
      .mockResolvedValueOnce({
        applied: true,
    ...
    await userEvent.click(screen.getAllByRole('button', { name: /이름 변경/ })[0])
    ...
    expect(ontologyApi.renameOption).toHaveBeenCalledWith('part', 'material', {
      from: '스틸',
      to: '강',
      apply: false,
    })
    ...
    await waitFor(() =>
      expect(ontologyApi.renameOption).toHaveBeenLastCalledWith('part', 'material', {
        from: '스틸',
        to: '강',
        apply: true,
      }),
  ...
  it('승격 — 계획(값마다 새로/재사용, 옮길 수)을 보고 누른다', async () => {
    ...
    ontologyApi.promoteProperty
      .mockResolvedValueOnce(plan)
      .mockResolvedValueOnce({ ...plan, applied: true, snapshot_id: 's' })
    ...
    expect(ontologyApi.promoteProperty).toHaveBeenCalledWith('part', 'material', {
      target_type_slug: 'material_book',
      apply: false,
    })
