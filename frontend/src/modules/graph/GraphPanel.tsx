 ...
 *
 * ReportArchive 의 `EntityGraphPanel` 을 옮겨 왔다. 2단계까지만 그리고, 노드를
...
export function GraphPanel({ objectId, typeSlugs }: GraphPanelProps) {
  ...
  if (!graph.data || graph.data.edges.length === 0) return null

  const typeOf = new Map(graph.data.nodes.map((one) => [one.id, one.type_slug]))
    ...
    <section className="space-y-2">
      ...
      <GraphCanvas
        ...
        onNodeClick={(id) => {
          ...
          const slug = typeOf.get(id)
          if (slug) navigate(`/o/${slug}/${id}`)
        }}
      />
    </section>
  )
