...
부서 이름, 계정 표시 이름, 그리고 앞으로 도메인이 들일 모든 "사람이 타이핑하는
이름" 이 같은 문제를 갖는다: 눈에 같아 보이는데 DB 는 다르게 보는 값들이다.
`재료팀` 과 `재료팀 `(뒤 공백), `UTM-01` 과 `utm-01`. 비교키를 만드는 규칙이
모듈마다 다르면 어떤 화면은 중복을 막고 어떤 화면은 못 막는다.
"""
