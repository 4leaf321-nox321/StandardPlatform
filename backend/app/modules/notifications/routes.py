...
def unread_count(
    ...
    """상단 배지가 주기적으로 부른다. **접근 로그에서 뺀 경로다**(shared/access_log.py)
    — 안 빼면 표가 이 한 줄로 가득 찬다."""
...
def mark_read(
    ...
    # **남의 알림은 없는 것과 같이 답한다.** 403 으로 가르면 그 id 가 존재한다는
    # 사실이 새고, 그것은 알려 줄 이유가 없는 정보다.
    if row is None or row.user_id != user.id:
        raise NotFound(code("NOTIFICATIONS", 1), "알림을 찾을 수 없습니다.")
