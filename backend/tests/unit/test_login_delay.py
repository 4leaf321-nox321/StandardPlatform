...
관리자 복구가 서버 콘솔뿐인 시스템에서 잠금은 자해다. 늦추기만 해도 무차별
시도는 시간당 몇 번으로 줄고, 비밀번호를 아는 사람은 한 번 기다리면 된다.
"""
...
def test_문턱_전에는_안_늦춘다() -> None:
    """오타 한두 번에 기다리게 하면 정상 사용자가 먼저 다친다."""
    after = get_settings().login_delay_after
    for failures in range(1, after):
...
def test_상한을_넘지_않는다() -> None:
    """무한히 늘면 정상 사용자가 복구할 방법이 사라진다 — 그것은 잠금과 같다."""
    settings = get_settings()
