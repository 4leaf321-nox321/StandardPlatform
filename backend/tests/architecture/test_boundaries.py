...
FRONTEND_MERGED: set[str] = {
    ...
    "bundles",
    # RDF/OWL 내보내기 — 화면은 온톨로지 › 가져오기 페이지의 링크뿐이라 모듈을 안 둔다.
    "rdf",
    # 코어 API — **바깥 시스템이 쓰는 창구**라 이 설치의 화면이 없다. 여는 것은 타입의 칸
    # 하나(온톨로지 › 타입)라 그 화면이 이미 있다. 누가 코어를 관리하는 화면(열린 것 ·
    # 누가 언제 받아 갔나)을 만들면 그 모듈을 만들고 여기서 뺀다.
    "coreapi",
}
